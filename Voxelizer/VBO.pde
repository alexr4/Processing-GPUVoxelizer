...
public class VBOInterleaved implements ShaderSource {
  ...
  //instanced VBO
  final static short INSTANCE_NBR_COMP    =  VEC4_CMP * 2; //position, scale of the instance
  ...
  final static short INSTANCE_SCL_OFFSET  =    (VEC4_CMP * 1) * Float.BYTES;
  ...
  public void initInstanceVBO(PGL pgl, int vboID, FloatBuffer buffer) {
    this.offsetBufferId = vboID;
    this.offsetBuffer = buffer;
    pgl.bindBuffer(GL2ES2.GL_ARRAY_BUFFER, offsetBufferId);
    // transfer data to VBO, this perform the copy of data from CPU -> GPU memory
    pgl.bufferData(GL.GL_ARRAY_BUFFER, this.offsetBuffer.limit() * Float.BYTES, this.offsetBuffer, GL.GL_DYNAMIC_DRAW);
  }

  public void updateVBO() {
    this.updateMeshVBO();
    this.updateInstanceVBO();
  }
  
  public void updateMeshVBO() {
    this.updateVBO(this.interleavedBuffer, this.VBOi);
  }
  ...
  public void draw(PGraphics context) {
    ...
    //fill data
    gl.glBufferData(PGL.ARRAY_BUFFER, Float.BYTES * this.VBOi.length, interleavedBuffer, PGL.DYNAMIC_DRAW);//USE PGL.STATIC_DRAW if attributes are not set to be update by the CPU
    ...
    int scaleLoc = gl.glGetAttribLocation(this.shader.glProgram, "scale");
    gl.glEnableVertexAttribArray(scaleLoc);

       // bind VBO
    gl.glBindBuffer(PGL.ARRAY_BUFFER, offsetBufferId);
    //fill data is not necessary when you use compute shader because data are already bound in the initInstanceVBO function
    //gl.glBufferData(PGL.ARRAY_BUFFER, Float.BYTES * pcl.datas.length, pcl.pclbuffer, PGL.DYNAMIC_DRAW);
    
    //Associate current bound vbo with attribute
    gl.glVertexAttribPointer(offsetLoc, VEC4_CMP, PGL.FLOAT, false, INSTANCE_STRIDE, INSTANCE_VRT_OFFSET);
    gl.glVertexAttribPointer(scaleLoc, VEC4_CMP, PGL.FLOAT, false, INSTANCE_STRIDE, INSTANCE_SCL_OFFSET);
    gl.glVertexAttribDivisor(offsetLoc, 1);
    gl.glVertexAttribDivisor(scaleLoc, 1);
    ...
    gl.glVertexAttribDivisor(scaleLoc, 0);
    ...
    gl.glDisableVertexAttribArray(scaleLoc);
